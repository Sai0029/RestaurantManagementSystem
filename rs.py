...
def reset():
    ...
    textthickshake.config(state=DISABLED)
...
def receipt():
    ...
    if costoffoodvar.get() != '' or costofcakesvar.get() != '' or costofdrinksvar.get() != '':
        ...
        if e_fruitjuice.get() != '0':
            textReceipt.insert(END, f'fruitjuice:\t\t\t{int(e_fruitjuice.get()) * 30}\n\n')

        if e_thickshake.get() != '0':
            textReceipt.insert(END, f'thickshake:\t\t\t{int(e_thickshake.get()) * 40}\n\n')

        if e_milkshake.get() != '0':
            textReceipt.insert(END, f'milkshake:\t\t\t{int(e_milkshake.get()) * 60}\n\n')
...
def totalcost():
    ...
    if var1.get() != 0 or var2.get() != 0 or var3.get() != 0 or var4.get() != 0 or var5.get() != 0 or \
        ...
        item13 = int(e_fruitjuice.get())
        item14 = int(e_thickshake.get())
        item15 = int(e_milkshake.get())
...
def fruitjuice():
    if var13.get() == 1:
        textfruitjuice.config(state=NORMAL)
        textfruitjuice.focus()
        textfruitjuice.delete(0, END)
    elif var13.get() == 0:
        textfruitjuice.config(state=DISABLED)
        e_fruitjuice.set('0')
...
def thickshake():
    if var14.get() == 1:
        textthickshake.config(state=NORMAL)
        textthickshake.focus()
        textthickshake.delete(0, END)
    elif var14.get() == 0:
        textthickshake.config(state=DISABLED)
        e_thickshake.set('0')
...
root.geometry('1470x840+0+0')

root.resizable(0,0)

root.title('Restaurant Management System created by Nagineni Sai Krishna')
...
rightFrame=Frame(root,bd=15,relief=RIDGE,bg='red')
rightFrame.pack(side=RIGHT)

calculatorFrame=Frame(rightFrame,bd=1,relief=RIDGE,bg='red')
calculatorFrame.pack()

recieptFrame=Frame(rightFrame,bd=4,relief=RIDGE,bg='cyan')
recieptFrame.pack()

buttonFrame=Frame(rightFrame,bd=3,relief=RIDGE,bg='cyan')
buttonFrame.pack()
...
fruitjuice=Checkbutton(drinksFrame,text='fruitjuice',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var13
                     ,command=fruitjuice)
fruitjuice.grid(row=3,column=0,sticky=W)

thickshake=Checkbutton(drinksFrame,text='thickshake',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var14
                     ,command=thickshake)
thickshake.grid(row=4,column=0,sticky=W)

milkshake=Checkbutton(drinksFrame,text='milkshake',font=('arial',18,'bold'),onvalue=1,offvalue=0,variable=var15
                      ,command=milkshake)
...
textfruitjuice = Entry(drinksFrame, font=('arial', 18, 'bold'), bd=7, width=6, state=DISABLED, textvariable=e_fruitjuice)
textfruitjuice.grid(row=3, column=1)

textthickshake = Entry(drinksFrame, font=('arial', 18, 'bold'), bd=7, width=6, state=DISABLED, textvariable=e_thickshake)
textthickshake.grid(row=4, column=1)
